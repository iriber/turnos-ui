...
/**
 * 
 * Finder para profesionales.
 * 
 * @author bernardo
 * @since 13/08/2013
 */
class ProfesionalFinder implements  IAutocompleteFinder {
	...
	public function findEntitiesByText( $text, $parent=null ){
		...
		$uiCriteria = new UIProfesionalCriteria();
		$uiCriteria->setNombre( $text );
		$uiCriteria->setRowPerPage( 10 );
		return UIServiceFactory::getUIProfesionalService()->getList($uiCriteria);	
	}
	...
	function findEntityByCode( $code, $parent=null ){
		...
		return UIServiceFactory::getUIProfesionalService()->get( $code );
	}
